...
func TestNewFrictionEvent(t *testing.T) {
	tests := []struct {
		...
		{name: "unknown command failure", kind: FailureUnknownCommand},
		{name: "unknown flag failure", kind: FailureUnknownFlag},
		{name: "invalid arg failure", kind: FailureInvalidArg},
		{name: "parse error failure", kind: FailureParseError},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			event: &FrictionEvent{
				...
				PathBucket: "repo",
				Input:      "mycli agent prime --unknownflag",
